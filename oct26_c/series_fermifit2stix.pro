...
f_fit = spex_read_fit_results('ospex_results_27_jul_2022.fits')
...
fermi_dim = size(fermi_ct_rate, /dimensions)
fermi_ct_flux = fltarr(fermi_dim)
fermi_ct_flux_err = fltarr(fermi_dim)
fermi_norm = f_fit.SPEX_SUMM_AREA*fermi_get_edges.width      
for i=0, fermi_dim[1]-1 do begin
  fermi_ct_flux[*, i] = fermi_ct_rate[*, i]/fermi_norm
  fermi_ct_flux_err[*, i] = fermi_ct_rate_err[*, i]/fermi_norm
endfor
;----------------------------------------------------;
...
save, out, filename="stix_spec_fermi_firsttest.sav"
